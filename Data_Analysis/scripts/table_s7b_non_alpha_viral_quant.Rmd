---
title: "repo_table_s7b_non_alpha_viral_quant"
author: "Jianyu Lai"
date: "7/16/2021"
...
set.seed(42)
```
## data: nonpaired pcr_dat, filtered for seronegative participants
```{r}
...
TableS7b <- table_df %>% rename(`Sample Type`=sample_type,`Samples`="qPCR Samples",`Maximum RNA copies`="Max_ave_per_sample")
TableS7b[TableS7b=="NA (NA)"] <- "-"
TableS7b <- TableS7b %>% select(`Sample Type`,Cases,`Participants LOD`,`Participants LOQ`,Samples,`Samples LOD`,`Samples LOQ`,`Culture n/N* (% Positive)`,`GM (95% CI)`,GSD,`Maximum RNA copies`)

# rename and reorder rows
TableS7b$`Sample Type`[TableS7b$`Sample Type`=="Phone Swab"] <- "Fomite"
tables7border <- c("Midturbinate Swab","Saliva","Fomite","G-II Coarse Aerosol","G-II Coarse Aerosol mask","G-II Fine Aerosol","G-II Fine Aerosol mask")

TableS7b_1 <- tables7border %>% matrix(ncol=1) %>% as.data.frame(stringsAsFactors=F)
colnames(TableS7b_1) <- "Sample Type"
TableS7b_1 <- TableS7b_1 %>% left_join(TableS7b,by="Sample Type")
write.csv(TableS7b_1,file="../working_files/repo_table_s7b_semiformatted.csv",row.names=F)
```

```{r fix the scientific notation}
TableS7b_1$`Maximum RNA copies` <- TableS7b_1$`Maximum RNA copies` %>% sapply(fixRNAscientific)
TableS7b_1$`GM (95% CI)` <- TableS7b_1$`GM (95% CI)` %>% sapply(GMCIparse_fixsci)
TableS7b_1
```

```{r write final table}
write.csv(TableS7b_1, file="../table_output/repo_table_s7b_non_alpha_viral_quant.csv",row.names = F)
```
