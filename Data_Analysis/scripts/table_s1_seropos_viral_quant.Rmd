---
title: "table_s1_seropos_viral_quant"
author: "Jianyu Lai"
date: "7/15/2021"
output: html_document
---
Derived from Table 2 script (viral quant analysis of seronegative cases)
```{r setup,include=F}
...
## data: nonpaired pcr_dat, filtered for seropositive participants
```{r}
nonpaired_pcr_dat <- read.csv("../working_files/nonpaired_pcr_sx_rpr.csv",stringsAsFactors=F)
dF <- nonpaired_pcr_dat %>% filter(pos_enrollment=="Positive")
...
```{r}
ef_est_ML_df_list <- list()

pcrdatclean_sampletypes1 <- pcrdatclean_sampletypes %>% subset(!(grepl("IcePac|G-II",.)))
...
for (q in pcrdatclean_sampletypes1) {
  ...
  #### Take model parameter estimates to generate GM (GSD) and confidence interval ####
  betas <- length(fit_ML$beta) 
  ...
  for (i in 1:betas) {
    ...
    ef_est_ML <- rbind(ef_est_ML, fold_inc_var)
  }
  
  ef_est_ML_df <- as.data.frame(ef_est_ML) %>%
                  ...
                  mutate(gsd = signif(exp(fit_ML$sigma), digits = 2))
  
  print(ef_est_ML_df)
  ef_est_ML_df_list[[q]] <- ef_est_ML_df

}
```

fill in blanks for G-II sample types
```{r}
V1 <- "intercept"
effect_ci <- "-"
gsd <- "-"
ef_est_ML_df <- cbind(V1, effect_ci, gsd)
ef_est_ML_df <- as.data.frame(ef_est_ML_df,stringsAsFactors=F) %>% mutate(gsd = as.double(gsd))
ef_est_ML_df_list[["G-II Fine Aerosol"]] <- ef_est_ML_df
ef_est_ML_df_list[["G-II Fine Aerosol mask"]] <- ef_est_ML_df
ef_est_ML_df_list[["G-II Coarse Aerosol"]] <- ef_est_ML_df
ef_est_ML_df_list[["G-II Coarse Aerosol mask"]] <- ef_est_ML_df
```

## Final GM table
```{r}
gm_table1 <- bind_rows(ef_est_ML_df_list)
gm_table1$`Sample Type` <- names(ef_est_ML_df_list)
...
## Bind summary_wo_gm with gm_table1 
```{r}
...
TableS1 <- table_df %>% rename(`Sample Type`=sample_type,`Samples`="qPCR Samples",`Maximum RNA copies`="Max_ave_per_sample")
TableS1 <- TableS1 %>% select(`Sample Type`,Cases,`Participants LOD`,`Participants LOQ`,Samples,`Samples LOD`,`Samples LOQ`,`Culture n/N* (% Positive)`,`GM (95% CI)`,GSD,`Maximum RNA copies`)

# rename and reorder rows
TableS1$`Sample Type`[TableS1$`Sample Type`=="Phone Swab"] <- "Fomite"
tables1order <- c("Midturbinate Swab","Saliva","Fomite","G-II Coarse Aerosol","G-II Coarse Aerosol mask","G-II Fine Aerosol","G-II Fine Aerosol mask")

TableS1$`Participants LOD`[TableS1$`Participants LOD`=="NA (NA)"] <- "0 (0)"
TableS1$`Participants LOQ`[TableS1$`Participants LOQ`=="NA (NA)"] <- "0 (0)"
TableS1$GSD[is.na(TableS1$GSD)] <- "-"
TableS1$`Maximum RNA copies`[is.na(TableS1$`Maximum RNA copies`)] <- "-"

TableS1_1 <- tables1order %>% matrix(ncol=1) %>% as.data.frame(stringsAsFactors=F)
colnames(TableS1_1) <- "Sample Type"
TableS1_1 <- TableS1_1 %>% left_join(TableS1,by="Sample Type")
```

```{r fix the scientific notation}
TableS1_1$`Maximum RNA copies` <- TableS1_1$`Maximum RNA copies` %>% sapply(function(x) {if (x!="-") {fixRNAscientific(x)} else {x}})
TableS1_1$`GM (95% CI)` <- TableS1_1$`GM (95% CI)` %>% sapply(function(x) {if (x!="-") {GMCIparse_fixsci(x)} else {x}})
TableS1_1
```

```{r write final table}
write.csv(TableS1_1, file="../table_output/repo_table_s1_seropos_viral_quant.csv",row.names = F)
```
